...
export default function TransactionsTable(props: TransactionTableProps) {
  ...
  const { rows, isLoading, isError, isFetching, maxRows = 3 } = props;
  ...
  const tableState = isFull
    ? {
      ...
      pagination: { pageIndex: props.page - 1, pageSize: props.pageSize },
    }
    : { globalFilter: props.search };
  ...
  const table = useReactTable({
    data: displayRows,
    columns,
    state: tableState as Partial<TableState>,
